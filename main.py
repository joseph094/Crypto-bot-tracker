...
response = requests.get("https://newsapi.org/v2/everything?q=crypto&apiKey=dad5f41659b94aa3ad8401c39fc3c60b")
...
async def on_message(message):
    ...
    if message.channel.name=='general':
        if user_message.lower() == 'hello':
            ...
            response= f'This is ur random number {random.randrange(10)} '
...
async def on_message(message):
    ...
    if message.content.startswith('$news'):
        count = 0
        await message.channel.send(f"Hey! {message.author.name}, check your DMs for the todays Top 5 news articles")
